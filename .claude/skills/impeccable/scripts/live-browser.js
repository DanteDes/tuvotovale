...
(() => {
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    if (isJsxSourceFile(filePath)) {
      ...
      if (liveWrapper?.querySelector('[data-impeccable-variant]:not([data-impeccable-variant="original"])')) {
  ...
  function startScrollLock(sessionId, initialTargetY, initialAnchorTop) {
    stopScrollLock();
    scrollLockTargetY =
      typeof initialTargetY === "number" && Number.isFinite(initialTargetY) ? initialTargetY : window.scrollY;
    scrollLockAnchorTop =
    ...
    const correct = (_why) => {
      ...
      const anchor = resolveBarAnchor();
      if (anchor?.isConnected && typeof scrollLockAnchorTop === "number" && Number.isFinite(scrollLockAnchorTop)) {
